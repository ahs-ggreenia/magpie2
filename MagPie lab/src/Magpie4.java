...
public class Magpie4
{
	...
	public String getResponse(String statement)
	{
                    ...
                    || findKeyword(statement, "brother") >= 0)
		{
			response = "Tell me more about your family.";
                        ...
                        || findKeyword(statement, "tiger") >=0
                        || findKeyword(statement, "bunny") >=0
                        || findKeyword(statement, "bear") >=0){
                 response = "Tell me more about your pet.";
                }else if(findKeyword(statement,"poltics") >= 0 
                        || findKeyword(statement, "Trump") >=0
                        || findKeyword(statement, "Biden") >=0
                        || findKeyword(statement, "democrat") >=0
                        || findKeyword(statement, "republican") >=0
                        || findKeyword(statement, "government") >=0
                        || findKeyword(statement, "election") >=0){
                 response = "What did you think of the election?";
              
                }else if(findKeyword(statement, "videogames") >=0
                        || findKeyword(statement, "gamer") >=0
                        || findKeyword(statement, "videogamer") >=0
                        || findKeyword(statement, "gaming") >=0
                        || findKeyword(statement, "videogaming") >=0)
                {
                 response = "So you like to game do you?";   
                    
                }
                else if(findKeyword(statement, "apple") >= 0
                        ||findKeyword(statement, "orange") >=0
                        ||findKeyword(statement, "banana") >=0
                        ||findKeyword(statement, "grape") >=0){
                    response = "so you like food right?";
		...
		else if (findKeyword(statement, "I want to", 0) >= 0)
		{
			response = transformIWantToStatement(statement);
                ...
                else if (findKeyword(statement, "I want to go", 0) >= 0)
		{
			response = transformIWantToGoStatement(statement);
		}
                else if (findKeyword(statement, "I want to go", 0) >= 0)
		{
	...
	private String transformYouMeStatement(String statement)
	{
        ...
        private String transformIWantToGoStatement(String statement)
	{
		//  Remove the final period, if there is one
		statement = statement.trim();
		String lastChar = statement.substring(statement
		...
		int psn = findKeyword (statement, "I want to go", 0);
		String restOfStatement = statement.substring(psn + 12).trim();
		return "How are you going to get to " + 
                        restOfStatement + "?";
        }
